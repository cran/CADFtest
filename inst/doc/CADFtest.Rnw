...
\Plaintitle{Unit Root CADF Testing with R}


%% an abstract and keywords
\Abstract {
  This introduction to the \pkg{CADFtest} package is a (slightly)
  modified version of \citet{lupi2009b}, published in the
  \emph{Journal of Statistical Software}. \pkg{CADFtest} is an
  \proglang{R} package for testing for the presence of a unit root
  in a time series using the Covariate Augmented Dickey-Fuller (CADF)
  test proposed in \citet{hansen1995}. The procedures presented here
  are user friendly, allow fully automatic model specification,
  and allow computation of the asymptotic $p$~values of the test.
}
...
As already mentioned, the {\em finite sample} $p$~value is computed using \code{punitroots()} implemented in package \pkg{urca} \citep{pfaff2008}. In principle, it would have been possible also to compute the {\em asymptotic} $p$~value by using the function \code{CADFpvalues} to be described in detail in the next section by invoking

<<>>=
CADFpvalues(ADFt$statistic, type="trend", rho2=1)
@

When a standard Dickey-Fuller test is performed, \code{CADFtest()} acts as an interface to existing commands. For example, in the case above equation (\ref{trend}) is estimated using the package \pkg{dynlm} \citep{zeileis2009} and the test $p$~value is computed using \code{punitroots()}.  
...
\pkg{fUnitRoots} \citep{wuertz2009} is another important \proglang{R} package performing unit root tests. In particular, the function \code{unitrootTest()} performs the ADF test and computes the relevant finite sample $p$~values using the approach developed in \citet{mackinnon1996}. However, no automatic lag selection is performed. The object passed to the procedure must be a vector or a time series, but no data argument is used, so that the usual call would be something like
...
The functions illustrated in this paper have been implemented in
\proglang{R}~\Sexpr{paste(R.Version()[6:7], collapse = ".")} \citep{R2009}
using the following packages, listed in alphabetical order:
\pkg{dynlm}~packageDescription("dynlm")["Version"] \citep{zeileis2009},
\pkg{sandwich}~packageDescription("sandwich")["Version"] \citep{zeileis2004, zeileis2006},
\pkg{tseries}~packageDescription("tseries")["Version"] \citep{trapletti2009},
\pkg{urca}~packageDescription("urca")["Version"] \citep{pfaff2008}. 
